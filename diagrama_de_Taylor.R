...
estaciones_depurado_completo <- homogenizacion_entre_db_observado_y_simulado(estaciones, solo_observado = TRUE)
eqm_depurado_completo <- homogenizacion_entre_db_observado_y_simulado(estaciones, eqm)
pqm_depurado_completo <- homogenizacion_entre_db_observado_y_simulado(estaciones, pqm)
gpqm_depurado_completo <- homogenizacion_entre_db_observado_y_simulado(estaciones, gpqm)
loci_depurado_completo <- homogenizacion_entre_db_observado_y_simulado(estaciones, loci)
ptr_depurado_completo <- homogenizacion_entre_db_observado_y_simulado(estaciones, ptr)
qdm_depurado_completo <- homogenizacion_entre_db_observado_y_simulado(estaciones, qdm)
era5_depurado_completo <- homogenizacion_entre_db_observado_y_simulado(estaciones, era5)

eqm_depurado <- homogenizacion_entre_db_observado_y_simulado(estaciones, eqm, para_Taylor = TRUE, pbias = FALSE)
pqm_depurado <- homogenizacion_entre_db_observado_y_simulado(estaciones, pqm, para_Taylor = TRUE, pbias = FALSE)
gpqm_depurado <- homogenizacion_entre_db_observado_y_simulado(estaciones, gpqm, para_Taylor = TRUE, pbias = FALSE)
loci_depurado <- homogenizacion_entre_db_observado_y_simulado(estaciones, loci, para_Taylor = TRUE, pbias = FALSE)
ptr_depurado <- homogenizacion_entre_db_observado_y_simulado(estaciones, ptr, para_Taylor = TRUE, pbias = FALSE)
qdm_depurado <- homogenizacion_entre_db_observado_y_simulado(estaciones, qdm, para_Taylor = TRUE, pbias = FALSE)
era5_depurado <- homogenizacion_entre_db_observado_y_simulado(estaciones, era5, para_Taylor = TRUE, pbias = FALSE)
...
diagrama.de.Taylor.modificado.2(R.i = metodos$r[1], sd.ref.i = metodos$sd.observado[1], sd.model.i = metodos$sd.simulado[1],
                                ...
                                pch = pch.bias, show.gamma = TRUE, ancho.linea.de.pch = 0.5, main = '', xlab = 'Standard deviation')
...
colorlegend(db.bias.y.color$color.bias, intervalos.para.leyenda, xlim = c(10.7, 11.7), ylim = c(0.5, 10))
text(11, 10.4, 'Bias', srt = 0) # 'Bias' en t2,  'Bias (%)' en pp
legend(0.5, 12, legend=leyenda.i, horiz=FALSE, cex = 0.71, ncol = 4, lty = NULL, text.font = 2)
# text(0, 11, '(b)', font = 2) # '(a)' en t2, '(b)' en pp

# colorlegend(db.bias.y.color$color.bias, intervalos.para.leyenda, xlim = c(10.7, 11.7), ylim = c(0.5, 10))
# text(11.8, 5, 'Bias', srt = 270) # 'Bias' en t2,  'Bias (%)' en pp
